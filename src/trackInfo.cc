...
trackInfo::~trackInfo(){
  delete tree_;
  delete Track_Vposition ;
  delete Track_3Momentum ;
}
...
void trackInfo::Fill(const edm::Event& iEvent){
  ...
  for(track = tkColl.begin(); track != tkColl.end(); track++) {
    //if(track->pt()>1.){
      ...
      //std::cout<<"track eta:phi:::::::::"<<track->eta()<<" \t"<< track->phi()<< std::endl;
      TVector3 v3(track->vx(),track->vy(),track->vz());
      new( (*Track_Vposition)[Track_n]) TVector3(v3);
      Track_n++;
      //}
  }
  //std::cout<<"Track_n:"<<Track_n<<std::endl;
  int numhighpurity=0;
...
void trackInfo::SetBranches(){
  ...
  AddBranch(&Track_3Momentum, "Track_3Momentum");
